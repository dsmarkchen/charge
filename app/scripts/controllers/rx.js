...
/**
 * @ngdoc function
 * @name chargeApp.controller:RxCtrl
 * @description
 * # RxCtrl
...
angular.module('chargeApp')
  ...
  .controller('RxCtrl', function ($scope, $http, $filter) {
       ...
       var url3 = 'https://dsmarkchen.github.io/charge/chargepinsky.txt';
       var url4 = 'https://dsmarkchen.github.io/charge/chargebang.txt';
       ...
       $http.get(url2).then(function (rsp) {
            ...
            $scope.charge = rsp.data.split(/\r?\n/) ;
            for(var i = 0; i < $scope.charge.length; i++) {
               $scope.buildone($scope.charge[i], i, usingBreaker);
            } 
            if(temp.length > 0) {   
